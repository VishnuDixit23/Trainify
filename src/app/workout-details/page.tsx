...
const WorkoutDetailsPage: React.FC = () => {
  ...
  return (
    ...
    > 
      ...
      >
        <h3 className="text-2xl text-white font-semibold mb-3">Important Considerations</h3>
        <ul className="list-disc ml-5 text-stone-300 text-xl space-y-4">
          {existingPlan.workoutPlan.important_considerations.map((item: any, index: number) => (
            <li key={index} className="hover:text-white  transition-colors duration-200">
        ...
        {Array.isArray(existingPlan.workoutPlan.warm_up) ? (
          <ul className="text-stone-300 mt-3 space-y-4">
            {existingPlan.workoutPlan.warm_up.map((warmup: any, index: number) => (
              <li key={index} className="hover:text-white text-xl transition-colors duration-200">
        ...
        {existingPlan.workoutPlan.schedule.map((day: any, dayIndex: number) => (
          ...
          >
            ...
            {day.workouts.map((workout: any, index: number) => (
              <div key={index} className="text-stone-300 text-xl mt-4">
                <p className="hover:text-white transition-colors duration-200">
                  <strong className=" font-bold text-2xl mr-4"
                  >{workout.exercise}  :</strong> ({workout.reps}) reps × {" "}
                  {workout.sets} sets
                </p>
              </div>
            ))}
          ...
          <button
            ...
            className="mt-8  w-1/5 py-3 px-6 bg-red-400 text-stone-600 font-semibold rounded-lg shadow-md hover:bg-red-500 transform hover:scale-105 focus:outline-none"
          >
            Delete Workout Plan
          </button>
        </motion.div>
...
>
  ...
  Provide your details, and our AI-powered system will generate a personalized workout plan tailored to your goals, fitness level, and preferences.
</p>
