...
export default function CreateRoutinePage() {
  ...
  const saveRoutine = async () => {
    ...
    const routineData = {
      ...
      exercises: Object.entries(selectedExercises).flatMap(
        ([day, exercises]) => exercises.map((ex) => ({ ...ex, day })) // Attach day key to each exercise
      ),
    };
  ...
  return (
    <div className="flex flex-col min-h-screen p-8 bg-gradient-to-br from-stone-900 via-black to-stone-800 text-stone-300">
      {existingRoutine ? (
        <motion.div className="flex-1 bg-gradient-to-br from-stone-800 via-stone-900 to-black p-6 rounded-2xl shadow-lg w-full max-w-full mx-auto">
          <h2 className="text-3xl font-bold text-stone-300 mb-6">
            Existing Routine
          </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 place-items-center">
                {exercises.map((ex: Exercise, index: number) => (
                  ...
                  >
                    ...
                    />
                    <p className="font-extrabold text-2xl text-stone-300">
                      {ex.name}
                    </p>
        ...
        <>
          <div className="flex gap-8 p-6">
            <motion.div
              className="flex-1 bg-gradient-to-br from-stone-700 via-stone-800 to-black  p-8 rounded-2xl shadow-lg"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            >
              <h2 className="text-3xl font-bold text-stone-300 mb-16">
                Create Routine
              </h2>
              <input
                type="text"
              ...
              <div className="border rounded-lg p-6 min-h-[550px] bg-stone-50 mt-14 text-stone-500">
                {selectedExercises[currentDay]?.length ? (
                  ...
                  <p className="text-gray-500 text-center py-6 font-semibold">
                    No Exercises Added
                  </p>
                )}
              </div>
            ...
            {/* Exercise Library */}
            <motion.div
              className="w-1/3  bg-gradient-to-br from-stone-700 via-stone-800 to-black p-8 rounded-3xl shadow text-stone-500"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            >
              <h3 className="text-2xl font-bold text-stone-300 mb-7">
                Exercise Library
              </h3>
              <input
                type="text"
